...
public class RestaurantAgent extends GuiAgent {
	...
	protected void setup() {
		...
		System.out.println(" La capacité actuelle de ce restaurant est " + capacite);
		...
		guiEvent.addParameter(" La capacité actuelle de ce restaurant  est"  + capacite);
				...
				try {
					...
					if (aclMessage != null) {
						if (nbplacesDispo != 0) {
							...
							GuiEvent guiEvent = new GuiEvent(this, 1);
							guiEvent.addParameter(" Une demande de réservation de  " + message + " ,Nombre de place disp :" + nbplacesDispo );
							
							//guiEvent.addParameter("  Le Nombre de place disp :" + nbplacesDispo);
							Platform.runLater(() -> {
								gui.insertMessage(guiEvent);
							});
							//gui.insertMessage(guiEvent);
	...
	public void takeDown() {
		GuiEvent guiEvent = new GuiEvent(this, 1);
		guiEvent.addParameter("Malheureseument le restaurant : " + this.getAID().getLocalName() + " est saturé pour le moment ");
		Platform.runLater(() -> {
			gui.insertMessage(guiEvent);
		});
		//gui.insertMessage(guiEvent);
